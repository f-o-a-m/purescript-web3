...
import Network.Ethereum.Web3.Solidity (UIntN, Tuple0(..), Tuple1(..), class IndexedEvent, unTuple1)
...
type ConstructorFn = Tagged Void Tuple0
...
newtype CountSet = CountSet { _count :: UIntN 256 }
...
instance IndexedEvent Tuple0 (Tuple1 (Tagged "_count" (UIntN 256))) CountSet where
  isAnonymous _ = false

newtype Deployed = Deployed { _blockNumber :: UIntN 256 }
...
instance IndexedEvent Tuple0 (Tuple1 (Tagged "_blockNumber" (UIntN 256))) Deployed where
  isAnonymous _ = false

type CountFn = Tagged "count()" Tuple0
...
type SetCountFn = Tagged "setCount(uint256)" (Tuple1 (Tagged "_count" (UIntN 256)))

setCount :: TransactionOptions NoPay -> { _count :: UIntN 256 } -> Web3 HexString
setCount x1 x2 = uncurryFields x2 $ setCount' x1
  where
  setCount' :: TransactionOptions NoPay -> Tagged "_count" (UIntN 256) -> Web3 HexString
