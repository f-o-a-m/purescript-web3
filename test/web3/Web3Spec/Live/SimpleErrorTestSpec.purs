...
spec provider =
  ...
  describe "SimpleError" do
    it "can raise a left for unset values" $ \cfg -> do
      let {contractAddress: simpleErrorTestAddress, userAddress} = cfg
          txOptions = defaultTestTxOptions # _to ?~ simpleErrorTestAddress
                                           # _from ?~ userAddress
          n = mkUIntN s256 1
