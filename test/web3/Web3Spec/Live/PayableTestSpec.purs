...
spec provider =
  ...
  describe "PayableTest" do
    it "can send the right amount of Ether" $ \cfg -> do
      let {contractAddress: payableTestAddress, userAddress} = cfg
          txOptions = defaultTestTxOptions # _to ?~ payableTestAddress
                                           # _value ?~ convert (mkValue one :: Value Ether)
                                           # _from ?~ userAddress
          etherAction = PayableTest.seeContent txOptions
      Tuple _ (PayableTest.Content c) <- assertWeb3 provider $
        takeEvent (Proxy :: Proxy PayableTest.Content) payableTestAddress etherAction
      unUIntN c._paidContent `shouldEqual` one


    it "can send the right amount of Shannon" $ \cfg -> do
      let {contractAddress: payableTestAddress, userAddress} = cfg
          txOptions = defaultTestTxOptions # _to ?~ payableTestAddress
                                           # _value ?~ convert (mkValue one :: Value Shannon)
                                           # _from ?~ userAddress
          etherAction = PayableTest.seeContent txOptions
      Tuple _ (PayableTest.Content c) <- assertWeb3 provider $
        takeEvent (Proxy :: Proxy PayableTest.Content) payableTestAddress etherAction
