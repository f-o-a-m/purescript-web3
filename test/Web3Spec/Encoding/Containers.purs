...
staticArraysTests =
    describe "statically sized array tests" do
      ...
      it "can encode statically sized vectors of addresses" do
         let mgivenElement = toVector s1 $ [false]
      ...
      it "can encode statically sized vectors of statically sized vectors of type bool" do
         let mgiven = toVector s2 $ map (\a -> unsafePartial fromJust $ mkAddress =<< mkHexString a) [ "407d73d8a49eeb85d32cf465507dd71d507100c1"
      ...
      it "can encode statically sized vectors of statically sized bytes" do
         let elem1 = unsafePartial fromJust (fromByteString s1 =<< flip BS.fromString BS.Hex "cf")
  ...
  describe "tuples test" do
    ...
    it "can do something really complicated" do
      let uint = unsafePartial $ fromJust $ uIntNFromBigNumber s256 $ embed 1
          ...
          ints = [ unsafePartial $ fromJust $ intNFromBigNumber s256 $ embed 1
          ...
          given = Tuple9 uint int bool int224 bools ints string bytes16 bytes2s :: Tuple9 (UIntN S256)
                                                                                          (IntN S256)
                                                                                          Boolean
                                                                                          (IntN S224)
                                                                                          (Vector S2 Boolean)
