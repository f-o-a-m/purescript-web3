...
type ModuleImportsAcc
  = { types :: Map ModuleName IsCtrInImports, imports :: Array String }

runImported :: GeneratorOptions -> FilePath -> Imported String -> String
runImported opts destFile c =
  let
    (Tuple code accImports) = runWriter c
  in
...
runImports = mergeImports >>> map runImport >>> newLine1 >>> ("import Prelude \n\n" <> _)
  ...
  runModuleImports = runAcc <<< foldl f { types: mempty, imports: mempty }
    where
    ...
    f acc = case _ of
      IType a ->
        if member a acc.types then
          acc
        else
          acc { types = insert a CtrIsNotInImports acc.types }
      ITypeCtr a -> case lookup a acc.types of
        Nothing -> { types: insert a CtrIsInImports acc.types, imports: acc.imports <> [ a <> "(..)" ] }
        Just CtrIsInImports -> acc
        Just CtrIsNotInImports -> { types: insert a CtrIsInImports acc.types, imports: acc.imports <> [ a <> "(..)" ] }
      ITypeOp a -> acc { imports = acc.imports <> [ "type (" <> a <> ")" ] }
      IClass a -> acc { imports = acc.imports <> [ "class " <> a ] }
      IVal a -> acc { imports = acc.imports <> [ a ] }
      IOp a -> acc { imports = acc.imports <> [ "(" <> a <> ")" ] }
...
generatePS os = do
  let
    opts = os { pursDir = os.pursDir <> "/" <> replaceAll (Pattern ".") (Replacement "/") os.modulePrefix }
    ...
    fs' -> do
      errs <-
        join
          <$> for fs' \f -> do
              let
                f' = genPSFileName opts f
              Tuple _ errs <- runWriterT $ writeCodeFromAbi opts f f'
              liftEffect
                $ log
                    if null errs then
                      successCheck <> " contract module for " <> f <> " successfully written to " <> f'
                    else
                      warningCheck <> " (" <> show (length errs) <> ") contract module for " <> f <> " written to " <> f'
              pure errs
      unless (null errs) do
...
newtype ABIError
  = ABIError { abiPath :: FilePath, idx :: Int, error :: String }
