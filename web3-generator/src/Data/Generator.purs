...
type Imports
  = Array (Tuple ModuleName ModuleImports)
...
funToTypeDecl fun@(SolidityFunction f) opts = do
  factorTypes <-
    if f.isUnCurried then
      for f.inputs tagInput
    else
      for f.inputs $ \(FunctionInput fi) -> toPSType fi.type
  pure
    $ FunTypeDecl
  ...
  let
    ...
    stockVars =
      if f.isConstructor then
        [ var <> "0", if isWhereClause then "bc'" else "bc" ]
      else if f.constant then
        [ var <> "0", if isWhereClause then "cm'" else "cm" ]
      else
        [ var <> "0" ]
...
toTransportPrefix isConstructor isCall outputCount = do
  fun <-
    if isConstructor then do
      ...
      pure "sendTx"
  modifier <-
    if isCall && outputCount == 1 then do
...
toPayload isWhereClause typeName args = do
  ...
  let
    tupleType = "Tuple" <> show (length args)
  import' "Network.Ethereum.Web3.Solidity" [ ITypeCtr tupleType ]
...
instance codeHelperFunction :: Code HelperFunction where
  genCode (CurriedHelperFunction h) opts =
    let
      constraints = fold $ map (\c -> c <> " => ") h.constraints
    ...
    in
      pure <<< fold $ map (\s -> indentation <> s) [ decl <> "\n", defL <> " = " <> defR ]
    where
...
eventToDataDecl (SolidityEvent ev) = do
  ...
  indexedTypes <-
    for is \(IndexedSolidityValue sv) -> do
      t <- toPSType sv.type
      pure $ Tuple sv.name t
  nonIndexedTypes <-
    for nis \(IndexedSolidityValue sv) -> do
      t <- toPSType sv.type
      pure $ Tuple sv.name t
  recordType <-
    for ev.inputs \(IndexedSolidityValue sv) -> do
      t <- toPSType sv.type
      pure $ Tuple sv.name t
  pure
    $ EventDataDecl
...
instance codeEventDataDecl :: Code EventDataDecl where
  genCode (EventDataDecl decl) _ = do
    ...
    let
      recordField (Tuple label val) = label <> " :: " <> val
...
instance codeEventGenericInstance :: Code EventGenericInstance where
  genCode (EventGenericInstance i) _ =
    ...
    in
      pure $ newLine2 $ i.genericDeriving : instances
...
instance codeEventDecodeInstance :: Code EventDecodeInstance where
  genCode (EventDecodeInstance ev) _ = do
    ...
    let
      indexedEventDecl = "instance indexedEvent" <> ev.combinedType <> " :: IndexedEvent " <> ev.indexedTuple <> " " <> ev.nonIndexedTuple <> " " <> ev.combinedType <> " where"
...
eventId (SolidityEvent e) =
  ...
  in
    fromByteString $ keccak256 $ e.name <> paren (joinWith "," eventArgs)
...
eventToEventFilterInstance ev@(SolidityEvent e) = do
  ...
  mkFilterExpr addr = do
    ...
    let
      ...
      indexedVals =
        if nIndexedArgs == 0 then
          ""
        else
          "," <> joinWith "," (replicate nIndexedArgs "Nothing")
...
mkComment cs =
  let
    sep = (fromCharArray $ replicate 80 '-') <> "\n"
  in
    sep <> (newLine1 $ map (\s -> "-- | " <> s) cs) <> "\n" <> sep
...
funToFunctionCodeBlock fun@(SolidityFunction f) opts = do
  ...
  helperFunction <-
    if f.isUnCurried then
      funToHelperFunction' fun opts
    else
      funToHelperFunction false fun opts <#> CurriedHelperFunction
...
instance codeFunctionCodeBlock :: Code CodeBlock where
  ...
  genCode (EventCodeBlock decl@(EventDataDecl d) filterInst eventInst genericInst) opts = do
    let
      header = mkComment [ d.constructor ]
    declCode <- genCode decl opts
