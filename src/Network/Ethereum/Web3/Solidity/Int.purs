...
-- | See module [Network.Ethereum.Web3.Solidity.Sizes](/Network.Ethereum.Web3.Solidity.Sizes) for some predefined sizes.
intNFromBigNumber :: forall n. KnownSize n => Proxy n -> BigNumber -> Maybe (IntN n)
intNFromBigNumber _ a
  | a < zero =
    let
      minVal = negate $ (embed 2) `pow` (sizeVal (Proxy :: Proxy n) - one)
    in
      if a < minVal then Nothing else Just <<< IntN $ a
  | otherwise =
    let
      maxVal = (embed 2) `pow` (sizeVal (Proxy :: Proxy n) - one) - one
    in
